...
public class HazelcastQueryCreator extends AbstractQueryCreator<KeyValueQuery<Predicate<?, ?>>, Predicate<?, ?>> {
    ...
    private Predicate<?, ?> from(Part part, Iterator<Comparable<?>> iterator) {
        ...
        switch (type) {
            ...
            case CONTAINING:
                return fromContainingVariant(type, property, iterator);
            case STARTING_WITH:
                return fromStartWithVariant(type, property, iterator);
            case REGEX:
                return fromRegexVariant(type, property, iterator);
            /* case AFTER:
             * case BEFORE:
			 * case BETWEEN:
			 * case ENDING_WITH:
    ...
    private Predicate<?, ?> fromRegexVariant(Type type, String property, Iterator<Comparable<?>> iterator) {
    ...
    private Predicate<?, ?> fromInVariant(Type type, String property, Iterator iterator) {
        Object value = iterator.next();
        if (value instanceof List) {
            List<String> values = (List<String>) value;
            return Predicates.in(property, values.toArray(new String[]{}));
        }
        throw new InvalidDataAccessApiUsageException(String.format("Logic error for '%s' in query (value for IN must be list of String", type));
    }

    private Predicate<?, ?> fromContainingVariant(Type type, String property, Iterator<Comparable<?>> iterator) {
        switch (type) {
            case CONTAINING:
                return Predicates.like(property, "%" + iterator.next().toString() + "%");
    ...
    private Predicate<?, ?> fromStartWithVariant(Type type, String property, Iterator<Comparable<?>> iterator) {
        switch (type) {
            case STARTING_WITH:
                return Predicates.like(property, iterator.next().toString() + "%");
